...
- [ ] 1. Scaffold project foundation and design system
  - [ ] 1.1 Initialise Next.js 14+ App Router project with TypeScript strict mode and Tailwind CSS
  ...
  - [ ] 1.3 Install and configure `next/font` for Geist Sans
  ...
  - [ ] 1.4 Create `src/types/index.ts` with all Sanity-shaped TypeScript interfaces
  ...
  - [ ] 1.5 Create `src/data/nav.ts` with static navigation config
  ...
  - [ ]* 1.6 Create `src/data/seed/` directory with development seed data
  ...
  - [ ] 1.7 Set up `.env.local` with all required environment variables
...
- [ ] 2. Build layout components
  - [ ] 2.1 Build `RootLayout` (`src/app/layout.tsx`) and `Footer` component
  ...
  - [ ] 2.2 Build `Nav` component with desktop dropdown and mobile hamburger
  ...
  - [ ]* 2.4 Write property test for Nav href mapping (Property 2)
  ...
  - [ ] 2.5 Build `PageHeader`, `Divider`, `Button`, `BackLink`, `CategoryBadge` shared primitives
...
- [ ] 3. Build shared content row components
  - [ ] 3.1 Build `ExperienceRow` component typed against Sanity `ExperienceEntry`
  ...
  - [ ]* 3.2 Write property test for ExperienceRow (Property 5)
  ...
  - [ ] 3.3 Build `ProjectRow` component typed against Sanity `Project`
  ...
  - [ ]* 3.4 Write property test for ProjectRow alternating layout (Property 11)
  ...
  - [ ]* 3.5 Write property test for ProjectRow required fields (Property 10)
  ...
  - [ ] 3.6 Build `BlogPostRow` component typed against Sanity `BlogPostPreview`
  ...
  - [ ]* 3.7 Write property test for BlogPostRow (Property 13)
...
- [ ] 4. Build homepage sections and homepage page
  - [ ] 4.1 Build `Hero` and `MetadataRow` components
  ...
  - [ ] 4.3 Build `ExperiencePreview`, `AboutEditorial`, `BlogPreview`, `ContactCTA` homepage section components
  ...
  - [ ] 4.4 Wire homepage (`src/app/page.tsx`) with seed data for layout validation
...
- [ ] 5. Build experience, projects, and about pages
  - [ ] 5.1 Wire `/experience` page (`src/app/experience/page.tsx`)
  ...
  - [ ] 5.4 Build project detail components and wire `/projects/[slug]` page
  ...
  - [ ] 5.6 Wire `/about` page (`src/app/about/page.tsx`)
...
- [ ] 6. Build blog pages
  - [ ] 6.1 Wire `/blog` page (`src/app/blog/page.tsx`)
    - Compose `PageHeader "Writing"`, then map all post items to `<BlogPostRow>` with dividers
    - Use seed data for layout validation; mark for Sanity wiring in Task 8
    - `export const revalidate = 60`
    - _Requirements: 7.1, 7.2, 7.3_

  - [ ] 6.2 Build `ArticleHeader` and `ArticleBody` blog components
  ...
  - [ ] 6.3 Wire `/blog/[slug]` page (`src/app/blog/[slug]/page.tsx`)
...
- [ ] 7. Checkpoint — layout and component pass
...
- [ ] 8. Sanity CMS — schema definitions
  - [ ] 8.1 Create Sanity project and configure dataset
  ...
  - [ ] 8.2 Define `project` schema (`sanity/schemas/project.ts`)
  ...
  - [ ] 8.3 Define `experience` schema (`sanity/schemas/experience.ts`)
  ...
  - [ ] 8.4 Define `skill` schema (`sanity/schemas/skill.ts`)
    - Fields: `label` (string, required), `order` (number)
    - Add preview config
    - _Requirements: 6.8, 11.3_

  - [ ] 8.5 Define `post` schema (`sanity/schemas/post.ts`)
  ...
  - [ ] 8.6 Define `siteSettings` singleton schema (`sanity/schemas/siteSettings.ts`)
  ...
  - [ ] 8.7 Register all 5 schemas in `sanity.config.ts`
  ...
  - [ ]* 8.8 Write property test for all Sanity schema field completeness (Property 14)
...
- [ ] 9. Sanity CMS — client, GROQ queries, and data wiring
  - [ ] 9.1 Implement Sanity client (`src/lib/sanity/client.ts`)
  ...
  - [ ] 9.2 Implement all GROQ queries (`src/lib/sanity/queries.ts`)
  ...
  - [ ]* 9.3 Write property test for projects page renders all published projects (Property 7)
  ...
  - [ ]* 9.4 Write property test for systems page category filter (Property 8)
  ...
  - [ ]* 9.5 Write property test for media page category filter (Property 9)
  ...
  - [ ] 9.6 Wire `/projects` page to `ALL_PUBLISHED_PROJECTS_QUERY` with ISR
    - Replace seed data with `sanityClient.fetch(ALL_PUBLISHED_PROJECTS_QUERY)` in `src/app/projects/page.tsx`
    - `export const revalidate = 60`
    - _Requirements: 5.11_

  - [ ] 9.7 Wire `/projects/systems` and `/projects/media` pages with ISR
  ...
  - [ ] 9.8 Wire `/projects/[slug]` with `PROJECT_BY_SLUG_QUERY` + `generateStaticParams` + `notFound()`
  ...
  - [ ] 9.9 Wire `/experience` page to `ALL_EXPERIENCE_QUERY` with ISR
    - Replace seed data with `sanityClient.fetch(ALL_EXPERIENCE_QUERY)` in `src/app/experience/page.tsx`
    - `export const revalidate = 60`
    - _Requirements: 4.6_

  - [ ] 9.10 Wire `/blog` page to `ALL_POSTS_QUERY` with ISR
    - Replace seed data with `sanityClient.fetch(ALL_POSTS_QUERY)` in `src/app/blog/page.tsx`
    - `export const revalidate = 60`
    - _Requirements: 7.1_

  - [ ] 9.11 Wire `/blog/[slug]` with `POST_BY_SLUG_QUERY` + `generateStaticParams` + `notFound()`
  ...
  - [ ] 9.12 Wire `/about` page to `SITE_SETTINGS_QUERY` + `ALL_SKILLS_QUERY` with ISR
  ...
  - [ ] 9.13 Wire homepage (`/`) to all 4 Sanity queries with ISR
...
- [ ] 10. Seed real content into Sanity CMS
  - [ ] 10.1 Seed all real content into Sanity Studio
...
- [ ] 11. Checkpoint — full Sanity wiring pass
...
- [ ] 12. Build contact form and API route
  - [ ] 12.1 Build `ContactForm` component with client-side validation
  ...
  - [ ]* 12.2 Write property test for ContactForm empty field validation (Property 17)
  ...
  - [ ]* 12.3 Write property test for ContactForm invalid email validation (Property 18)
  ...
  - [ ]* 12.4 Write property test for ContactForm field rendering (Property 16)
  ...
  - [ ] 12.5 Build `/api/contact` Route Handler with Resend
  ...
  - [ ] 12.6 Build `ContactInfo` component and wire `/contact` page
...
- [ ] 13. SEO, metadata, and Polish
  - [ ] 13.1 Add `generateMetadata()` to all page files
  ...
  - [ ] 13.2 Audit all images from Sanity CDN
  ...
  - [ ] 13.3 Full responsive pass and forbidden pattern audit
  ...
  - [ ] 13.4 Run `tsc --noEmit` and ESLint — fix all errors
    - Zero TypeScript errors, zero ESLint violations before deploy
    - _Requirements: 9.8_

- [ ] 14. Deployment and final validation
  - [ ] 14.1 Deploy to Vercel and set all environment variables
  ...
  - [ ] 14.2 Run Lighthouse on deployed URL and verify scores
  ...
  - [ ] 14.3 Final content and ISR verification
