...
export function Button({ href, children, secondary = false }: { href: string; children: React.ReactNode; secondary?: boolean }) {
  return <Link href={href} className={`inline-flex min-h-12 items-center justify-center px-6 text-sm font-bold transition-colors ${secondary ? "border border-[var(--border-strong)] hover:border-[var(--accent)] hover:text-[var(--accent)]" : "bg-[var(--accent)] text-[#0d0d0d] hover:bg-[var(--accent-strong)]"}`}>{children}</Link>;
}
