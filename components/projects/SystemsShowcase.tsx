  ...
  return (
    <div className="border-t border-white/10">
      {projects.map((project, index) => (
        <article
          key={project.slug}
          className="group grid gap-6 border-b border-white/10 py-10 md:grid-cols-[72px_minmax(0,1fr)_220px] md:gap-8 md:py-14"
        >
          ...
          <div>
            ...
            <h2 className="mt-4 max-w-3xl balanced text-[clamp(2rem,4vw,3.8rem)] font-black leading-[0.98] tracking-[-0.035em] text-white transition-colors duration-300 group-hover:text-[#e7e9ed]">
              {project.title}
            </h2>
          ...
          <div className="flex flex-col justify-between border-t border-white/8 pt-5 md:border-l md:border-t-0 md:pl-8 md:pt-0">
            ...
            <Link href={`/projects/${project.slug}`} className="mt-8 inline-flex w-fit border-b border-[var(--red)] pb-2 text-sm font-semibold text-white transition hover:text-[var(--red)]">
              Read case study <span className="arrow-shift ml-3" aria-hidden="true">→</span>
            </Link>
          </div>
        </article>
      ))}
