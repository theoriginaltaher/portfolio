...
export function AlbumCard({ album }: { album: MediaAlbum }) {
  return <article className="group border border-[var(--border)] bg-[var(--panel-muted)] transition-colors hover:border-[var(--border-strong)]">
    <Link href={`/projects/media/${album.slug}`} className="block">
      ...
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--blue-quiet)]">{album.category}</p><span className="shrink-0 text-sm text-[var(--dim)]">↗</span></div>
